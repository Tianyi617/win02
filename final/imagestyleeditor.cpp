    ...
    : QMainWindow(parent), scene(new QGraphicsScene(this)), m_undoStack(new QUndoStack(this))
{
    ...
    // 返回主菜單按鈕
    QPushButton *backButton = new QPushButton("返回主菜單", this);
    connect(backButton, &QPushButton::clicked, this, &ImageStyleEditor::returnToMainMenu);
...
void ImageStyleEditor::returnToMainMenu()
{
    emit backToMainMenu(); // 發送返回信號
}
